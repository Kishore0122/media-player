    ...
    try {
        ...
        try {
            ...
            if (response.ok) {
                ...
                console.warn(`Directory listing failed with status: ${response.status}. Using fallback.`);
            }
        } catch (fetchError) {
...
function playMusic(track, pause = false) {
    try {
        ...
        currentSong.onerror = (e) => {
            ...
            alert(`Failed to load audio: ${track}. Check if the file exists at ${songPath}`);
        };
...
async function displayAlbums() {
    try {
        ...
        const defaultAlbums = [
            { folder: "ncs", title: "NCS Releases", description: "No Copyright Sounds" },
            { folder: "romantic", title: "Romantic Songs", description: "Best romantic tracks" },
            { folder: "popular", title: "Popular Hits", description: "Top trending songs" }
        ];
        ...
        try {
            ...
            if (response.ok) {
                ...
                console.warn(`Album directory listing failed with status: ${response.status}. Using default albums.`);
            }
        } catch (fetchError) {
        ...
        const albumsToDisplay = fetchedAlbums.length > 0 ? fetchedAlbums : defaultAlbums;
        ...
        for (const album of albumsToDisplay) {
            ...
            cardContainer.innerHTML += `
                <div data-folder="${album.folder}" class="card">
                    ...
                    <img src="./songs/${album.folder}/cover.jpg" onerror="this.src='src/default-cover.jpg'" alt="Album Cover">
